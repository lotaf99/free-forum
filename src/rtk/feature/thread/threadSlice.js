...
export const threadsLoading = createSlice({
  name: 'threadload',
  initialState,
  reducers: {
    onLoadingThread: (state) => {
      state.isLoading = true;
      state.error = null;
      state.data = null;
    },
    onSuccessThread: (state, action) => {
      state.isLoading = false;
      state.data = action.payload.data;
    },
    onErrorThread: (state, action) => {
      state.isLoading = false;
...
export const { onLoadingThread, onSuccessThread, onErrorThread } = threadsLoading.actions;
...
export const asyncFetchThread = createAsyncThunk('thread/fetch', async (_, thunkApi) => {
  try {
    thunkApi.dispatch(showLoading());
    thunkApi.dispatch(onLoadingThread());
    const threads = await fetchThreads();
    thunkApi.dispatch(onSuccessThread({ data: threads.data }));
    return {
      data: threads,
      error: false,
    };
    ...
    thunkApi.dispatch(onErrorThread({ message }));
    return {
      message,
      error: true,
    };
