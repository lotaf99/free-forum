...
export const detailThreadsLoading = createSlice({
  name: 'detailthreadload',
  initialState,
  reducers: {
    onLoadingDetailThread: (state) => {
      state.isLoading = true;
      state.error = null;
      state.data = null;
    },
    onSuccessDetailThread: (state, action) => {
      state.isLoading = false;
      state.data = action.payload.data;
    },
    onErrorDetailThread: (state, action) => {
      state.isLoading = false;
...
export const { onLoadingDetailThread, onSuccessDetailThread, onErrorDetailThread } = detailThreadsLoading.actions;
...
export const asyncFetchDetailThread = createAsyncThunk('threadDetail/fetch', async (threadId, thunkApi) => {
  try {
    thunkApi.dispatch(showLoading());
    thunkApi.dispatch(onLoadingDetailThread());
    const detailThread = await fetchDetailThread(threadId);
    thunkApi.dispatch(onSuccessDetailThread());
    return {
      data: detailThread,
      error: false,
    };
    ...
    thunkApi.dispatch(onErrorDetailThread({ message }));
    return {
      message,
      error: true,
    };
